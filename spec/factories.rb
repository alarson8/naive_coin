module Factory
  def self.build(obj, options = {})
    case obj
    when :block
      default_options = { index: 1, previous_hash: "previous_hash", data: "data", difficulty: 0, nonce: 0 }
    ...
    when :block_hash
      ...
      Digest::SHA256.hexdigest("#{block.index}#{block.previous_hash}#{block.timestamp}#{block.data}#{block.nonce}")
    when :blockchain
    ...
    when :blockchain_with_blocks
      ...
      blockchain_length.times do |n|
        ...
        new_block = self.build(:block, { index: last_block.index + 1, previous_hash: last_block.hash, difficulty: BlockchainDifficulty.new(blockchain: blockchain).get, nonce: 0 })
